SSS = function(X,y,ind_fun,N=1000,C0=1,verbose=TRUE){
  n = nrow(X)
  p = ncol(X)
  r = 1
  tau = p^(2+0.05)/sqrt(n)
  ...
  #print(X[,ind2])
  #print(y)
  curr = ind_fun(X[,ind2], y, r, tau)   # evaluate the objective value on the initial model
  ...
  for(uu in 1:C0){ # repeat the SSS algorithm C0 times  
    ...
    for(iter in 1:N){ 
      ...
      if(verb==TRUE&iter%%50==0){
        print("#################################")
        curr = ind_fun(X[,ind2],y, r, tau)   # print the objective value of the current model
